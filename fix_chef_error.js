...
import { ChefHat, Sparkles, Loader2, Clock, Utensils, DollarSign, AlertCircle } from "lucide-react";
...
export default function ChefIA() {
  ...
  const generateRecipe = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || "Falha ao gerar receita");
      }
          ...
          {error && (
            <div className="bg-red-50 border border-red-100 p-6 rounded-2xl flex items-start gap-4 animate-in fade-in slide-in-from-top-2">
              ...
              <div className="space-y-1">
                <h3 className="font-bold text-red-900">Erro na Sugestão</h3>
                <p className="text-red-700 text-sm leading-relaxed">
                  {error.includes("quota") 
                    ? "Limite da API da OpenAI excedido. Verifique os créditos da sua chave de API." 
                    : error}
                </p>
              </div>
            </div>
...
`;
fs.writeFileSync('src/app/chef/page.tsx', content, 'utf8');
console.log('Chef page fixed with error handling');
